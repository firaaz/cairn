...
Always attempt the Write tool call. Do not refuse preemptively based on historical failure docs or prior-art you have read — invoke Write and report the actual outcome in the structured tail. If a write genuinely fails after you try, return `status:"RAISE_ISSUE"` with the concrete error the tool returned.
