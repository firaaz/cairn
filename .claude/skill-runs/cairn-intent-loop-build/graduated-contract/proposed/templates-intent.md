...
not by a clause-count check.
  ...
  GRADUATION. Depth scales with the work:
    - TRIVIAL (e.g. the cost-model m2 one-liner fix): scope-statement + a single
      bare `must-satisfy` clause. No premise grounding, no other clause-lists.
    - MEDIUM/HEAVY: the full six-clause grammar below, with `must-satisfy`
      clauses in EARS shape, plus `## Premise Grounding` for any claim the
      intent makes about existing source behaviour. Phase 1 drafts the clauses
      from the operator-approved promise — see `.claude/agents/phase-1-tdd.md`
      "Contract band".
...
check. A fully-absent `## Contract` block = fail-open with a visible stderr
notice, unless CAIRN_CONTRACT_REQUIRED=1. The mechanical gate does NOT enforce
the scope-statement floor or contract-depth — that is the decorrelation
...
Worked example — TRIVIAL (scope-statement + one bare clause): -->

```yaml
scope-statement: bump the cost-model m2 default so the light path stops over-firing
must-satisfy:
  - the validator reports the offending line number on a malformed id
```

<!-- Worked example — HEAVY (scope-statement + EARS clauses, one tagged
universal-set, plus the full clause-lists): -->

```yaml
scope-statement: regenerate every consumer dist mirror after the validator change
must-satisfy:
  - the validator reports the offending line number on a malformed id
