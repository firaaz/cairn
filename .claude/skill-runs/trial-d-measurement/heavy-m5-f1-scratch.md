# Trial-D measurement scratch — HEAVY (cairn-m5-f1-packaging)
...
Source intent: .claude/skill-runs/cairn-m5-f1-packaging/intent.md
  (completed-run record; copied below for derivation — do NOT edit the original).
Behaviors to derive from: acceptance criteria A1-A12 in `## Specification`.
  (A3/A4/A6 are universal-flavoured, A8 is a regression-meta clause — the measurement's
  named tag/split candidates. #1's authoring-time delta on THIS heavy intent is the
  named >30% breach test.)
