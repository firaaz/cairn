...
Verifies the four-ADR design decomposition of the feature-slice model:
  - semantic-identity (semantic identity), feature-slice-model (feature-slice model),
    parallelism-v1 (parallelism v1), context-tiers-integration (context tiers integration)
...
# Expected ADR files by flat-slug id (post-identifier-scheme D7 Phase 2 Part 1)
ADR_SLUGS = (
    "semantic-identity",
    "feature-slice-model",
    "parallelism-v1",
...
# phase-lock-and-role-declaration D4 excluded skills that parallelism-v1 must address
ADR004_D4_EXCLUDED_SKILLS = (
    "dispatching-parallel-agents",
    "using-git-worktrees",
)

# Additional phase-lock-and-role-declaration D4 exclusions (non-parallelism, may stay excluded)
...
def _find_adr(slug: str) -> Path | None:
    """Find the ADR file at docs/adr/{slug}.md (post-rename flat-slug)."""
...
def test_v1_four_adr_files_exist() -> None:
    """V1: Four ADR files exist at docs/adr/semantic-identity.md through docs/adr/context-tiers-integration.md."""
    for slug in ADR_SLUGS:
        path = _find_adr(slug)
        assert path is not None, (
            f"No ADR file matching docs/adr/{slug}.md found. (V1)"
        )
...
def test_v2_frontmatter_valid() -> None:
    ...
    for slug in ADR_SLUGS:
        path = _find_adr(slug)
        assert path is not None, f"ADR {slug}.md not found. (V2 pre-req)"
...
# ---------------------------------------------------------------------------
# V3: parallelism-v1 supersession references
# ---------------------------------------------------------------------------


def test_v3_adr007_supersession() -> None:
    """V3: parallelism-v1 supersedes: field names cliff-failure-mode-and-v1-defenses; prose explains partial scope."""
    path = _find_adr("parallelism-v1")
    assert path is not None, "ADR 007-*.md not found. (V3 pre-req)"
    text = _read(path)
    fm = _parse_frontmatter(text)
    assert fm is not None, "parallelism-v1 has no valid frontmatter. (V3 pre-req)"

    # Check supersedes field references cliff-failure-mode-and-v1-defenses
    ...
    adr003_referenced = any(
        "cliff-failure-mode-and-v1-defenses" in str(ref) or "003" in str(ref) for ref in all_supersession_refs
    )
    assert adr003_referenced, (
        f"parallelism-v1 frontmatter must reference cliff-failure-mode-and-v1-defenses in supersedes or "
    ...
    assert "partial" in body_lower or "d4" in text, (
        "parallelism-v1 prose must explain partial supersession scope "
        "(expected 'partial' or 'D4' reference in body). (V3)"
    )


# ---------------------------------------------------------------------------
# V4: parallelism-v1 addresses each phase-lock-and-role-declaration D4 excluded skill
# ---------------------------------------------------------------------------


def test_v4_adr007_addresses_adr004_d4() -> None:
    """V4: parallelism-v1 addresses each phase-lock-and-role-declaration D4 excluded skill — stays or returns."""
    path = _find_adr("parallelism-v1")
    ...
    for skill in ADR004_D4_EXCLUDED_SKILLS:
        assert skill in text, (
            f"parallelism-v1 must mention '{skill}' (un-excluded per supersession map). (V4)"
        )

    # parallelism-v1 should also address the remaining phase-lock-and-role-declaration D4 exclusions
    # (at minimum by referencing phase-lock-and-role-declaration D4 as a whole)
    addressed_other = any(skill in text for skill in ADR004_D4_OTHER_EXCLUSIONS)
    adr004_d4_ref = "phase-lock-and-role-declaration" in text and "D4" in text
    assert addressed_other or adr004_d4_ref, (
        "parallelism-v1 must address remaining phase-lock-and-role-declaration D4 exclusions — either by "
        "naming individual skills or referencing phase-lock-and-role-declaration D4 as a whole. (V4)"
    )


# ---------------------------------------------------------------------------
# V5: context-tiers-integration explicitly states INV-002 disposition
# ---------------------------------------------------------------------------


def test_v5_adr008_inv002_statement() -> None:
    """V5: context-tiers-integration explicitly states whether INV-002 needs amendment."""
    path = _find_adr("context-tiers-integration")
    assert path is not None, "ADR 008-*.md not found. (V5 pre-req)"
    text = _read(path)
    assert "INV-002" in text, "context-tiers-integration must reference INV-002. (V5)"
    ...
    assert confirms_no_change or specifies_amendment, (
        "context-tiers-integration must explicitly state whether INV-002 needs amendment — "
...
def test_v6_index_updated() -> None:
    """V6: docs/adr/index.md has entries for semantic-identity through context-tiers-integration."""
    text = _read(INDEX)
    for slug in ADR_SLUGS:
        adr_id = slug
...
def test_v8_no_implementation_decisions() -> None:
    ...
    *.md') are acceptable per ambiguity resolution A2. This test catches
    ...
    for slug in ADR_SLUGS:
        path = _find_adr(slug)
        assert path is not None, f"ADR {slug}.md not found. (V8 pre-req)"
        text = _read(path)
...
def test_adr_required_sections() -> None:
    ...
    for slug in ADR_SLUGS:
        path = _find_adr(slug)
        assert path is not None, f"ADR {slug}.md not found. (sections pre-req)"
        text = _read(path)
...
def test_adr007_provisional_risk_register() -> None:
    """parallelism-v1 (firmness: provisional) must have a Risk Register section."""
    path = _find_adr("parallelism-v1")
    assert path is not None, "ADR 007-*.md not found. (risk register pre-req)"
    text = _read(path)
    assert "## Risk Register" in text, (
        "parallelism-v1 is provisional and must have a '## Risk Register' section. (A6)"
    )
...
def test_adr005_firmness_firm() -> None:
    """semantic-identity firmness is firm per intent."""
    path = _find_adr("semantic-identity")
    assert path is not None, "ADR 005-*.md not found."
    fm = _parse_frontmatter(_read(path))
    assert fm is not None, "semantic-identity has no valid frontmatter."
    assert fm.get("firmness") == "firm", (
        f"semantic-identity firmness should be 'firm', got '{fm.get('firmness')}'"
    )


def test_adr006_firmness_firm() -> None:
    """feature-slice-model firmness is firm per intent."""
    path = _find_adr("feature-slice-model")
    assert path is not None, "ADR 006-*.md not found."
    fm = _parse_frontmatter(_read(path))
    assert fm is not None, "feature-slice-model has no valid frontmatter."
    assert fm.get("firmness") == "firm", (
        f"feature-slice-model firmness should be 'firm', got '{fm.get('firmness')}'"
    )


def test_adr007_firmness_provisional() -> None:
    """parallelism-v1 firmness is provisional per intent."""
    path = _find_adr("parallelism-v1")
    assert path is not None, "ADR 007-*.md not found."
    fm = _parse_frontmatter(_read(path))
    assert fm is not None, "parallelism-v1 has no valid frontmatter."
    assert fm.get("firmness") == "provisional", (
        f"parallelism-v1 firmness should be 'provisional', got '{fm.get('firmness')}'"
    )


def test_adr008_firmness_firm() -> None:
    """context-tiers-integration firmness is firm per intent."""
    path = _find_adr("context-tiers-integration")
    assert path is not None, "ADR 008-*.md not found."
    fm = _parse_frontmatter(_read(path))
    assert fm is not None, "context-tiers-integration has no valid frontmatter."
    assert fm.get("firmness") == "firm", (
        f"context-tiers-integration firmness should be 'firm', got '{fm.get('firmness')}'"
    )


# ---------------------------------------------------------------------------
# A5: context-tiers-integration token budget reference
# ---------------------------------------------------------------------------


def test_adr008_token_budget_reference() -> None:
    """context-tiers-integration references context-discipline-protocol's handoff token budget."""
    path = _find_adr("context-tiers-integration")
    assert path is not None, "ADR 008-*.md not found. (A5 pre-req)"
    text = _read(path)
    assert "context-discipline-protocol" in text, "context-tiers-integration must reference context-discipline-protocol. (A5)"
    text_lower = text.lower()
    budget_ref = "150" in text or "400" in text or "token" in text_lower
    assert budget_ref, (
        "context-tiers-integration must reference the handoff token budget "
        "(expected '150', '400', or 'token'). (A5)"
    )


# ---------------------------------------------------------------------------
# parallelism-v1: cliff-failure-mode-and-v1-defenses D4 parallelism items addressed
# ---------------------------------------------------------------------------


def test_adr007_addresses_adr003_d4_parallelism() -> None:
    """parallelism-v1 addresses cliff-failure-mode-and-v1-defenses D4's parallelism deferral."""
    path = _find_adr("parallelism-v1")
    assert path is not None, "ADR 007-*.md not found."
    text = _read(path)

    # Must reference cliff-failure-mode-and-v1-defenses and its parallelism deferral
    assert "cliff-failure-mode-and-v1-defenses" in text, "parallelism-v1 must reference cliff-failure-mode-and-v1-defenses."
    ...
    assert parallelism_addressed, (
        "parallelism-v1 must address parallelism (the core of cliff-failure-mode-and-v1-defenses D4 supersession)."
    )


# ---------------------------------------------------------------------------
# parallelism-v1: Supersession of phase-lock-and-role-declaration D4 also recorded in frontmatter
# ---------------------------------------------------------------------------


def test_adr007_supersedes_adr004_d4() -> None:
    """parallelism-v1 frontmatter references phase-lock-and-role-declaration supersession."""
    path = _find_adr("parallelism-v1")
    assert path is not None, "ADR 007-*.md not found."
    text = _read(path)
    fm = _parse_frontmatter(text)
    ...
    adr004_ref = any("phase-lock-and-role-declaration" in str(ref) or "004" in str(ref) for ref in all_refs)
    assert adr004_ref, (
        f"parallelism-v1 frontmatter must reference phase-lock-and-role-declaration in supersedes or "
