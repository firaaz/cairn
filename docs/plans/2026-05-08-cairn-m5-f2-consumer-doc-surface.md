...
- [ ] **Step 1 (Phase 2 — RED): existence + shape contract test.** Author `tests/unit/test_template_extraction.py` with one test per template, asserting:
  ...
  - `templates/intent.md` frontmatter contains keys `id`, `name`, `snapshot-sha`, `invariants-touched` (per the intent-system schema committed at `39f5e9f`; authoritative source is `.claude/agents/phase-1-tdd.md`). Body contains, in order, the eight section headings `## What`, `## Why`, `## Boundary`, `## Specification`, `## Verification`, `## Risk Surface`, `## Feature-Local Invariants`, `## Explicit Scope-Out`. The trailing three are required by the schema's derive-don't-fabricate contract; the template surfaces them so Phase 1 cannot omit them by oversight.
...
- [ ] **Step 3 (Phase 3 — GREEN): author `templates/intent.md`.** Frontmatter slot with the four schema-required keys (`id:`, `name:`, `snapshot-sha:`, `invariants-touched: []`) plus inline `<!-- -->` instruction comments naming what Phase 1 fills in for each. Body has the eight `## ` section headings in schema order: `## What`, `## Why`, `## Boundary`, `## Specification`, `## Verification`, `## Risk Surface`, `## Feature-Local Invariants`, `## Explicit Scope-Out` — each followed by a `<!-- -->` comment summarizing Phase 1's elicitation rule for that section (≤200-word combined budget for What/Why/Boundary; ≤80-word budget for Risk Surface; ≥1 entry for Feature-Local Invariants and Explicit Scope-Out; derive from plan-doc + cited ADRs + ARCHITECTURE.md only). Top pointer to `.claude/agents/phase-1-tdd.md` (authoritative) and `docs/operational-reference.md` "Phase 1" section.
