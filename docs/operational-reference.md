...
- **Branch-per-feature (D1).** Every feature runs on a `feat/<feature-id>` branch cut from `dev`, at the point that becomes the skill's `SNAPSHOT_SHA`. Instructed, not hook-enforced — a direct-to-`dev` commit stays INV-001-valid (operator-envelope-gated ad-hoc work is the legal fast path), but feature work belongs on a branch.
