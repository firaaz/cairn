...
This section is a **living registry** of the role, anti-behaviors, and recommended skills for each of the four phases. Unlike ADR-004 (which locks the phase count, the phase names, and the role names), updates to the skill mapping below do not require ADR supersession — they are ordinary documentation edits, analogous to how `docs/ARCHITECTURE.md` is regenerated by `/refresh-architecture`. ADR-004 (`docs/adr/004-phase-lock-and-role-declaration.md`) remains authoritative for the phase/role lock; this section is authoritative for the skill mapping and is read by `/catchup` and `/start-slice` at every phase entry.

### Role and anti-behaviors (from ADR-004 D2)
...
- `superpowers:using-git-worktrees` — worktrees are for parallelism or isolation from shared state; v1 single-slice work does not need them. Conditional in Phase 3 only if a subagent-driven task explicitly requires it.
