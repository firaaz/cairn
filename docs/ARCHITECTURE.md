...
**INV-010** Knowledge-substrate access for phase agents is mediated structurally. The cairn-knowledge MCP server (`mcp_servers/cairn_knowledge/`) wraps `scripts/cairn_query/` as four read-only tools (`lookup`, `search`, `path_bindings`, `cypher`); registered in `.mcp.json` under stdio transport; pinned per-phase via `AGENT_ENVELOPE.cairn_query_snapshot` set to `git rev-parse HEAD` at dispatch time. Direct Read/Bash/Grep/Glob access to `scripts/cairn_query/**`, `docs/ARCHITECTURE.md`, `docs/adr/**`, `docs/lessons.md`, `docs/spec-v1.md`, and `docs/operational-reference.md` is denied by `checks/role_guard.py`'s `ROLE_DENY_READ` table for all four phase roles — `AGENT_ROLE` in {`phase-1-writer`, `phase-2-skeptic`, `phase-3-implementer`, `phase-4-integrator`} — unless the active slice's `AGENT_ENVELOPE.paths` regex set grants the path (the grant escape hatch logs to `.claude/envelope-grants.log`). Operator memory under `~/.claude/projects/.../memory/` is permanently excluded from the MCP tool surface — no opt-in flag, no path-argument escape. Slice 3 (`compression/lever-Z-substrate-full-pipeline`) extended the lockdown from phase-1-writer (POC) to all four phase roles, completing the substrate program's v1 enforceability commitment. (cairn-substrate-and-fastmcp; compression-infrastructure-bootstrap)
...
**Slice-close lifecycle (slice-close-contract, firm; orchestrator-observability, provisional).** `scripts/slice_orchestrator/`'s `close_slice` function (in `lifecycle.py`) commits to INV-008's three operational invariants: idempotent close, sole producer of the `slice: complete` commit (with `run_phase_loop` skipping the Phase-4 boundary `commit_phase_handoff` call — the actual R3 source, confirmed during Phase 5 verification — and DC-4 defense layered via prompt constraint plus orchestrator `git log -1 --format=%s` defensive check), and cross-slice artifact isolation via slice-id-slug filenames with a collision tripwire. Resume reconciliation covers 13 state-triples with default-refuse on unrecognized combinations plus a heartbeat-staleness advisory. The companion orchestrator-observability ADR codifies the observability-artifact shape (JSON canonical at `<slug>-result.json` with MD sidecar at `<slug>-result.md` + append-only `index.jsonl` stream + `.heartbeat` liveness file) under a B2/B3-hybrid schema — firm internal fields (`slice_id`, `status`, `phase_timings`, `retries_by_phase`, `cluster_dispatches`, `degradation_reason`) alongside additive-safe forward-compat fields (`worktree_path`, `orchestrator_pid`) governed by a contract: adding optional fields does NOT bump `schema_version`; only removal or retyping does. Error policy is three-level (retry → degrade → strict-fail on self-observability); `degradation_reason` is append-preserving through terminal transitions. Retention is deferred behind a 100-file / 1-MB tripwire. The observability ADR is provisional and expected to iterate when the fleet-coordinator epic introduces its first real consumer.
