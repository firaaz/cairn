...
**INV-002** Session-to-session context transfer obeys a frontmatter contract on `.claude/handoff.md`: a YAML `contract:` block (`must-satisfy`, `must-not-violate`, `wrong-if`, `evidence`) declares the structural constraints, and the body is a pointer-only list where each line names a thread as `<pointer> <state> [<context>]` with `state ∈ {open, blocked, deferred}`. Trial A (interaction-protocol reframe, 2026-05-13, commit `53a7c58`) replaced the prior sectioned-narrative shape; per-feature plan docs at `docs/plans/<feature>.md` continue to carry session-spanning context. Machine-checkable binding via `test-ref` to `tests/unit/test_handoff_contract.py`. (context-discipline-protocol; invariant-binding-strategy)

```invariant-check INV-002
type: test-ref
pattern: "tests/unit/test_handoff_contract.py"
description: "Frontmatter contract on .claude/handoff.md — `contract:` block (must-satisfy/must-not-violate/wrong-if/evidence) plus pointer-only body where each thread declares state ∈ {open, blocked, deferred} with a resolvable pointer. Landed 2026-05-13 at commit 53a7c58 (Trial A), replacing the M4 sectioned-narrative shape. See docs/plans/2026-05-13-cairn-as-interaction-protocol.md."
```
...
**INV-012** Cairn's plugin payload deploys via a long-lived `release` branch with `marketplace.json` carrying `source: "url"` + HTTPS URL (`https://github.com/firaaz/cairn.git`) + `sha:` (40-char commit SHA pinning the consumer install to a specific `release` HEAD). The `release` branch's working-tree HEAD IS the curated dist payload (no `dist/` subdirectory; build output lands at branch root). The `sha:` shape (matching all 82 `url`-source plugins in Anthropic's `claude-plugins-official` marketplace) takes a fetch-by-commit branch over HTTPS in Claude Code's resolver, bypassing the `case 'github':` SSH-coercion path that breaks `ref:`-pinned shapes (V-3 attempts 1+2 falsified `ref:`; V-3 attempt 3 confirmed `sha:` works — empirical evidence at `.claude/skill-runs/cairn-m7-plugin-deployment-pattern/integration/sweep-notes.md` 2026-05-12 closure block). Consumer-visible update signal is `dist/.claude-plugin/plugin.json:version`; manual semver bumps per `m5-plugin-distribution-and-symlink-retire/D2`. CI release workflow is `workflow_dispatch`-triggered with version cross-check; force-with-lease replaces the `release` tree on each release; a sha-bump step on `dev` follows the `release` push (operator-manual until the workflow step lands per `plugin-payload-transport-a1/D3`). Update mechanism is documented as fragile (`/plugin update` does not refresh marketplace cache; consumers must `marketplace remove + re-add` per release per `plugin-payload-transport-a1/D5–D6`). (plugin-payload-transport-a1; m5-plugin-deployment-pattern; m5-plugin-distribution-and-symlink-retire)
