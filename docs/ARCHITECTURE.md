...
Two further substrate paths are reserved by slice-close-contract (firm) and orchestrator-observability (provisional): `.claude/orchestrator-debug/**` is cross-slice-persistent orchestrator-owned state — per-slice `<slug>-result.json`, `<slug>-result.md`, and `<slug>-phase-{N}-{role}-{ts}.log` failure logs, plus a shared append-only `index.jsonl` stream of failure and phase-3 cluster-dispatch events. `.claude/current-slice/.heartbeat` is ephemeral per-slice liveness written by a daemon thread; wiped by `close_slice`. `/integration-sweep` does not trim `.claude/orchestrator-debug/**`; any future rotation policy requires an explicit superseding slice per orchestrator-observability D8's tripwire (100 files OR 1 MB in the debug directory).
...
**Compression infrastructure bootstrap (compression-infrastructure-bootstrap, provisional).** `checks/role_guard.py` is narrowly authorized as a role-keyed write-path PreToolUse hook for the compression feature — keyed on `AGENT_ROLE`, no-op when unset, restricts `Write` / `Edit` / `MultiEdit` / `NotebookEdit` tools to the declared-output path set for each phase role plus `issue-triager`. Authorization is mechanism-specific: no other role-keyed hooks or gates (role-keyed `Read` gating, role-keyed subagent dispatch filters, etc.) are in scope. The hook is no-op under non-compressed execution, so existing non-compressed slices are unaffected. Expected to be superseded by the compression feature's Slice B Part 0 ADR (P1–P6 + D1/D2/D3); if Slice B stalls more than 10 slices, the compression feature itself is re-evaluated rather than this authorization promoted to firm.
...
**Board as roadmap substrate (board-as-roadmap-substrate, provisional).** Cairn's planning surface is layered as four stacked substrates: (1) the GH Project board (name `cairn`, account-owned) is the front-door catchment for un-committed ideas and roadmap items; (2) `.claude/features/<id>.yaml` is the slice-plan commitment register; (3) `.claude/current-slice/slice.yaml` + the four-phase pipeline is the execution engine; (4) `docs/lessons.md`, `.claude/handoff.md`, and operator memory are permanent / churning memory. The board is never a mirror of cairn-side state — it is its own truth (what could be worked on); cairn-side state remains derived per `feature-slice-model/d4-status-derived`. Cairn commands never call `gh project item-edit` directly; `/start-slice` and `/close-slice` append flip events to a local gitignored queue at `.claude/board-pending.log`, and a separate `.local/` reconciler (`/board-sync`) pushes the events to GH. This keeps the cairn pipeline offline-clean and structurally non-load-bearing on GH availability. `slice.yaml` schema additively gains optional `issue-number:` and `board-item-id:` fields; existing slices are not migrated. PM-session commands (`/groom`, `/promote`, `/weekly-status`, `/board-sync`) start in `commands/claude-code/.local/` per the `/dev-mode` precedent and do not ship to consumers. Parent-issue creation for features is **lazy** (at first-slice-open), not eager. `/promote`'s features.yaml write authority is **deferred** — features.yaml writes outside `/start-slice` are not currently authorized; future authorization requires a new ADR or amendment. The ADR is provisional with a dogfood gate at 10 slices using the system OR 2026-10-11 (whichever first); six failure-mode tripwires named in the ADR's risk register guide promotion, amendment, or supersession at the gate. Non-v1-scope per `cliff-failure-mode-and-v1-defenses/d4-time-box`.
