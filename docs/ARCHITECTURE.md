...
description: "Schema-shape lint binding for INV-012's marketplace.json contract: source.source==github, repo==firaaz/cairn, ref==release, no version on the marketplace plugin entry, no legacy type==git field. Five assertions in tests/unit/test_marketplace_schema.py (landed by m5-plugin-deployment-pattern's M7 implementing feature)."
```
