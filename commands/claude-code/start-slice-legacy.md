...
Fallback path. Invoke via `/start-slice --legacy` or env `CAIRN_LEGACY_START_SLICE=1`. Bypasses the orchestrator (`scripts/slice_orchestrator/` package, invoked as `PYTHONPATH=scripts uv run python -m slice_orchestrator`) and defers to the prose protocol preserved in `start-slice.full.md`.
