...
3. Run `uv run python scripts/integration_gate.py` — mechanizes invariant checking (delegates to `validate_architecture.py` Check D) and cross-module checks (ruff + pytest). Exit 0 = pass, 1 = fail, 2 = missing prerequisites.
4. Run `uv run python scripts/snapshot_diff.py --diff` — detects out-of-envelope file changes since last snapshot. Exit 0 = clean, 1 = out-of-envelope changes found. Then `uv run python scripts/snapshot_diff.py --snapshot` to update the baseline.
